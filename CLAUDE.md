...
- **Never** acquire a DB connection in nested calls. Use `_inner(&Connection)` helpers; public read fns check out `read_conn()`, public mutations lock `write_conn()`, and each passes one connection through (`src-tauri/src/db/mod.rs`).

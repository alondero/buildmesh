...
- Rust: `cargo test` / `cargo clippy --all-targets` (run inside `src-tauri/`). Prefer `--all-targets`: a lib-only run misses test-target lints, and cargo does **not** replay a cached crate's warnings — a no-op `cargo check` after an earlier warning-laden build prints nothing and reads as clean. Strict `--all-targets -- -D warnings` is still red today (issue #1491)
- **Frontend test env.** Run Vitest with `NODE_ENV=test`: if `NODE_ENV=production` is exported in the shell, every React render test fails with `React.act is not a function` (a harness error, not a code failure). A fresh worktree has **no** local `node_modules`, and nothing resolves from the main checkout — frontend tests fail on a missing Vite (`tests/integration/ui-shot*.test.ts` included) until you provide one. Fastest is a directory junction to the main checkout's install, run from the worktree root: `New-Item -ItemType Junction -Path node_modules -Target ..\..\..\node_modules` (three levels up; a worktree sits at `<repo>\.claude\worktrees\<name>`). Otherwise `npm install` here.

...
- **Never** acquire a DB connection in nested calls. Use `_inner(&Connection)` helpers; public read fns check out `try_read_conn()` (the typed [`DbError::ReaderPoolExhausted`] result), public mutations lock `write_conn()`, and each passes one connection through (`src-tauri/src/db/mod.rs`). A `#[cfg(test)]`-only `read_conn()` infallible accessor remains for hermetic tests where pool exhaustion is structurally impossible.

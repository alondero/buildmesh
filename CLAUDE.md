...
- **Never** acquire a DB connection in nested calls. Use `_inner(&Connection)` helpers; public read fns check out `try_read_conn()` (the typed [`DbError::ReaderPoolExhausted`] / [`DbError::NotInitialized`] result), public mutations lock `write_conn()`, and each passes one connection through (`src-tauri/src/db/mod.rs`). Tests call `try_read_conn().expect(...)` directly — there is no `#[cfg(test)]`-only infallible accessor because one was an attractive nuisance (#1533 review).

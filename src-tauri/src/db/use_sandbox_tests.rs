//! Tests for the per-mesh `use_sandbox` DB helper (`set_mesh_use_sandbox_inner`)
//! and the `ensure_mesh_use_sandbox` safety net (GH #498 — Windows AppContainer
//! sandbox; the toggle is shared with the macOS Seatbelt sibling #497).
//!
...
mod tests {
    ...
    /// Fresh in-memory schema with one mesh row, including the `use_sandbox`
    ...
    fn read(conn: &Connection, id: i64) -> bool {
        conn.query_row(
            "SELECT COALESCE(use_sandbox, 0) FROM meshes WHERE id = ?1",
            [id],
    ...
    fn defaults_to_false_for_freshly_created_mesh() {
        ...
        // validated (Phase 2/3). A fresh mesh must read back `false`.
    ...
    fn set_true_then_get_round_trips() {
        let (conn, id) = fresh_db_with_mesh();
        db::set_mesh_use_sandbox_inner(&conn, id, true).unwrap();
        assert!(read(&conn, id));
    }
    ...
    fn set_false_clears_the_flag() {
        let (conn, id) = fresh_db_with_mesh();
        db::set_mesh_use_sandbox_inner(&conn, id, true).unwrap();
        db::set_mesh_use_sandbox_inner(&conn, id, false).unwrap();
        assert!(!read(&conn, id));
    }
    ...
    fn set_for_unknown_mesh_id_errors() {
        ...
        let result = db::set_mesh_use_sandbox_inner(&conn, 999_999, true);
        assert!(
    ...
    #[test]
    fn ensure_mesh_use_sandbox_adds_column_and_defaults_existing_rows_to_false() {
        ...
        db::ensure_mesh_use_sandbox(&conn).unwrap();

        let col_present: bool = conn
            .query_row(
                "SELECT COUNT(*) > 0 FROM pragma_table_info('meshes') WHERE name = 'use_sandbox'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert!(col_present, "use_sandbox column should be added on first call");

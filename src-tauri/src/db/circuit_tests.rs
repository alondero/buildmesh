...
fn circuit_cleanup_retry_is_durable_and_excludes_borrowed_and_live_owners() {
    ...
    conn.execute(
        "UPDATE autopilot_circuit_runs SET context_json=json_set(context_json, '$.\"cleanup.claim.1\"', 'newer-generation') WHERE id=1",
        [],
    ).unwrap();
    ...
    let claim = super::circuit::claim_circuit_agent_cleanup_inner(&conn, 1).unwrap().unwrap();
    assert_eq!(claim, "newer-generation");
    assert_eq!(super::circuit::archive_circuit_agent_inner(&conn, 1, &claim).unwrap(), vec![(1, "failed".into())]);
...
#[test]
fn review_preset_history_is_not_dropped_by_the_recent_runs_limit() {
    let path = init_temp_db("review_history_unbounded");

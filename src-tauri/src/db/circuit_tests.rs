...
fn update_circuit_concurrency_limit_clamps_by_blueprint_and_errors_on_missing() {
    // Exercises the Tauri command directly (not just its DB primitive): the
    // blueprint floor must be applied and the returned row must report the
    // clamped value.
    let path = init_temp_db("cmd_set_concurrency");
    let mesh = create_mesh("circuit-cmd-concurrency-mesh", "/tmp/circuit-cmd-concurrency").unwrap();
    ...
    let created = create_autopilot_circuit(mesh.id, "review-loop", "", 1, &review_graph).unwrap();

    let updated =
        crate::commands::circuit::update_circuit_concurrency_limit(created.id, 1).unwrap();
    assert_eq!(updated.concurrency_limit, 2);
    assert_eq!(
        get_autopilot_circuit(created.id)
            .unwrap()
            .unwrap()
            .concurrency_limit,
        2,
    ...
    // A stale editor writing against a deleted circuit must error.
    let missing = crate::commands::circuit::update_circuit_concurrency_limit(999_999, 3);
    assert!(missing.is_err());

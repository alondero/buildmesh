...
pub fn advance(run: &mut RunView, event: &CircuitEvent) -> Transition {
    ...
    match event {
        ...
        CircuitEvent::WaitObserved {
            ...
            if !same_attempt || !same_mode || changed || since.is_none() {
                ...
                fail_step(
                    ...
                    node_id,
                    format!(
                        "Timed out after {} minutes without progress: {detail}",
                        timeout_ms / 60_000
                    ),
                );
                    ...
                    if classifier_outcome_is_routed(run, node_id, outcome) {
                        ...
                        if *classification == Classification::Continue && owns_target && count >= 2
                        {
                            fail_step(
                                ...
                                &mut t,
                                node_id,
                                "Automatic continuation budget exhausted after 2 attempts.".into(),
                            );
                            run.state = RunState::Failed;
                            t.run_state_changed = true;
                            finish_run_if_done(run, &mut t);
        ...
        CircuitEvent::GithubActionRetry { node_id } => {
            // A crash can land after a GitHub mutation but before its result
            // is committed. Re-emit the action for a still-running step;
            // review-blueprint OpenPr is idempotent because the worker first
            // looks up an existing PR for the branch.
            if matches!(
                run.step(node_id),
                Some(s) if s.status == StepStatus::Running
            ) {
                if let Some(CircuitNodeKind::GithubAction {
                    action,
                    label,
                    comment,
                    ..
...
fn set_step(run: &mut RunView, t: &mut Transition, node_id: &str, status: StepStatus) {
    ...
    if changed {
        ...
        t.step_writes
            .push(StepWrite::for_existing(node_id, status, attempt));
    }
}
...
mod tests {
    ...
    use crate::autopilot::circuit::model::{CircuitEdge, CircuitNode, GithubActionKind};
    ...
    fn canonical_walking_skeleton_lands_completed_through_pty_injection() {
        ...
        assert_eq!(
            run.state,
            RunState::Running,
            "PTY injection must execute before terminalising the run"
        );
    ...
    fn github_action_completes_instantly_and_emits_the_call_effect_with_templates() {
        ...
        run.attach_agent_node("spawn", 900);
        let _ = advance(&mut run, &agent_finished(900, true));
    ...
    #[test]
    fn running_github_action_can_be_replayed_after_a_crash() {
        let mut run = linear_run();
        run.graph.nodes.push(CircuitNode {
            id: "label".into(),
            kind: CircuitNodeKind::GithubAction {
        ...
        assert_eq!(status_of(&run, "label"), StepStatus::Running);
        assert_eq!(replay.effects, first.effects);
    }
    ...
    fn flaky_step_succeeding_on_retry_completes_the_run() {
        ...
        assert_eq!(run.step("work").unwrap().attempt, 2);
        let t = advance(&mut run, &agent_finished(11, true));
    ...
    fn close_agent_node_emits_a_targeted_close_effect() {
        ...
        let closed = advance(&mut run, &agent_finished(101, true));
        assert_eq!(
            closed.effects,
    ...
    fn issue_review_retry_exhaustion_runs_complete_notify_not_another_loop() {
        ...
        assert!(
            t.effects
                .iter()
                .any(|e| matches!(e, Effect::Notify { message } if message.contains("PR #1"))),

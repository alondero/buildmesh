//! Coordinator drive ledger GC (issue #750, item 3).
//!
...
//! meshes. The drive ledger GC is unrelated — it doesn't care about mesh
//! activity, doesn't iterate meshes, and the prune is one SQL DELETE rather
//! than a per-mesh walk. A dedicated worker keeps each module single-purpose
//! and the failure isolation obvious.
//!
//! ## Worker shape
//!
//! Mirrors `services::autopilot::start_autopilot_worker` (issue #482, PRD
//! #480): a `std::thread::spawn` whose body is
//! `loop { prune_once(); sleep(PRUNE_INTERVAL); }`. The very first iteration
//! runs immediately (no startup delay) — a single bounded DELETE is fast and
//! runs once on every app launch. Failures are logged and swallowed: a
//! transient SQLite error must not kill the worker thread (the next tick
//! re-tries).
...
/// runs forever in its own thread; failures inside `prune_once` are logged

...
//!
//!   1. `GetPlanInfo` — the provider-reported plan name.
...
//!      Enterprise accounts may omit it entirely.
//!   3. `get-aggregated-usage-events` — when an Enterprise account has no
//!      `planUsage`, aggregate the current cycle's spend events.
...
/// pinned while loopback tests point both at one `tiny_http` only if desired.
#[derive(Clone)]
...
enum CurrentFlow {
    ...
    /// The credential was rejected — do not attempt the legacy fallback, which
    /// would use the same credential.
    Unauthorized,
...
fn fetch_current_flow(client: &Client, token: &str, endpoints: &CursorEndpoints) -> CurrentFlow {
    // Step 1 — the account plan. Optional: a failed plan lookup still lets the
    // period-usage path run (and simply skips the Enterprise aggregate branch).
    let plan = match fetch_plan_name(client, token, endpoints) {
        Ok(plan) => plan,
        Err(CurrentFlow::Unauthorized) => return CurrentFlow::Unauthorized,
        Err(_) => None,
    };
    ...
    let enterprise = plan
        .as_deref()
        .is_some_and(|name| name.eq_ignore_ascii_case("enterprise"));

    // Step 3 — an Enterprise account with no plan-usage object reports spend
    // through the billing-cycle aggregate events instead. Best-effort: a
    // failure here leaves the response's own spend amounts as the fallback.
    let aggregate_spend_cents = if enterprise && !has_plan_usage {
...
/// `GetPlanInfo` → `planInfo.planName`. Returns `Ok(None)` for an
/// absent/unreadable plan (optional), `Err(Unauthorized)` when the credential
/// is rejected.
fn fetch_plan_name(
    client: &Client,
    token: &str,
    endpoints: &CursorEndpoints,
) -> Result<Option<String>, CurrentFlow> {
    let url = format!("{}{}", endpoints.api, PLAN_INFO_PATH);
    match read_connect_json::<CursorPlanInfoResp>(client, &url, token) {
        ...
        Err(ConnectError::Unavailable) => Ok(None),
    }
}
...
/// token JWT: `<userId>::<token>`, URL-encoded. Returns `None` when the token
/// is not a decodable JWT with a usable `sub` claim (the aggregate-events call
/// still proceeds with the bearer alone).
...
/// are never summed: a missing individual cap is `NoIndividualLimit`, not the
/// team pool's cap.
#[derive(Deserialize, Debug)]
#[serde(rename_all = "camelCase")]
...
impl CursorCycle {
    ...
    /// be in the future, so "current-cycle events" must stop at the present.
...
fn map_current_usage(
    ...
    let plan_usage = period.plan_usage.as_ref().filter(|usage| usage.is_usable());

    if let Some(plan_usage) = plan_usage {
        // Standard / Team / Ultra: the included allowance is the headline meter.
        meters.push(plan_usage.to_meter(&resets_at)?);
        if let Some(line) = plan_usage.breakdown_detail() {
            detail_parts.push(line);
        }
        if let Some(meter) = spend_meter(
        ...
        // No usable plan-usage object. Only Enterprise accounts fall through to
        // the current-cycle aggregate spend; other plans use the legacy probe.
        let enterprise = plan_name.is_some_and(|name| name.eq_ignore_ascii_case("enterprise"));
        if !enterprise {
            return None;
        }
    ...
    if let Some(line) = spend_limit_detail(period.spend_limit_usage.as_ref()) {
        detail_parts.push(line);
    }
    // Both branches above either pushed a meter or returned early, so `meters`
    // is non-empty here.
    Some(CursorMapped {
        meters,
...
impl CursorPlanUsage {
    ...
    fn to_meter(&self, resets_at: &Option<String>) -> Option<UsageMeter> {
        if let Some(limit_cents) = positive(&self.limit) {
            let used_cents = number(&self.total_spend)
                .or_else(|| number(&self.included_spend))
                .or_else(|| {
                    number(&self.remaining).map(|remaining| (limit_cents - remaining).max(0.0))
                })
                .unwrap_or(0.0);
            let remaining_cents =
                number(&self.remaining).or_else(|| Some((limit_cents - used_cents).max(0.0)));
            let used_percent = number(&self.total_percent_used)
                .or_else(|| Some((used_cents / limit_cents * 100.0).clamp(0.0, 100.0)));
            return Some(UsageMeter::Metered {
                amount: UsageAmount {
                    ...
                    unit: "USD".to_string(),
                    used_percent: used_percent.map(|percent| percent.clamp(0.0, 100.0)),
                    resets_at: resets_at.clone(),
                },
            });
        }
        let percent = number(&self.total_percent_used)?;
    ...
    fn breakdown_detail(&self) -> Option<String> {
        let included = number(&self.included_spend).map(cents_to_dollars);
        let bonus = number(&self.bonus_spend).map(cents_to_dollars);
...
/// Builds the spend-limit meter. `individual_only` is the Enterprise rule:
/// only an individual cap counts as a cap, so a team pool alone reports
/// `NoIndividualLimit` while still surfacing the pool in `detail`.
fn spend_meter(
    spend_limit_usage: Option<&CursorSpendLimitUsage>,
    aggregate_spend_cents: Option<f64>,
    ...
    let individual_limit = positive(&usage.individual_limit);
    let pooled_limit = positive(&usage.pooled_limit);

    let (limit_cents, used_cents, remaining_cents) = if individual_only {
        let limit = individual_limit;
        let used = number(&usage.individual_used)
            .or(aggregate_spend_cents)
            .or_else(|| number(&usage.total_spend));
        let remaining = number(&usage.individual_remaining);
        (limit, used, remaining)
    } else {
        let limit = individual_limit.or(pooled_limit);
        let used = number(&usage.individual_used)
            .or_else(|| number(&usage.pooled_used))
            .or_else(|| number(&usage.total_spend))
            .or(aggregate_spend_cents);
        let remaining =
            number(&usage.individual_remaining).or_else(|| number(&usage.pooled_remaining));
        (limit, used, remaining)
    };

    // A zero used amount is a valid reading (issue #1674 AC: "Zero Enterprise
    // spend is treated as a valid result") when the account reports spend at
    // all. Standalone zero placeholders on a standard plan stay suppressed so
    // an empty spend-limit object does not fabricate a meter.
    let usable = match limit_cents {
        Some(_) => true,
        None => used_cents.is_some_and(|used| individual_only || used > 0.0),
    };
    if !usable {
        return None;
    }
    let used_cents = used_cents.unwrap_or(0.0);
    let remaining_cents =
        remaining_cents.or_else(|| limit_cents.map(|limit| (limit - used_cents).max(0.0)));
    let used_percent = limit_cents
        .filter(|limit| *limit > 0.0)
        .map(|limit| (used_cents / limit * 100.0).clamp(0.0, 100.0));
    let amount = UsageAmount {
        used: cents_to_dollars(used_cents),
        limit: limit_cents.map(cents_to_dollars),
        remaining: remaining_cents.map(cents_to_dollars),
        unit: "USD".to_string(),
        used_percent,
        resets_at: resets_at.clone(),
    };
    Some(if limit_cents.is_some() {
...
/// Team-pool line for `detail`: the pooled limit is real information, but it is
/// not the individual cap, so it must not become the meter's limit.
fn spend_limit_detail(spend_limit_usage: Option<&CursorSpendLimitUsage>) -> Option<String> {
    let usage = spend_limit_usage?;
    let pooled_limit = positive(&usage.pooled_limit)?;
    let used = number(&usage.pooled_used).or_else(|| {
        number(&usage.pooled_remaining).map(|remaining| (pooled_limit - remaining).max(0.0))
    });
    Some(match used {
        Some(used) => format!(
            "Team pool spend {} of {}",
            money(cents_to_dollars(used)),
            money(cents_to_dollars(pooled_limit))
        ),
        None => format!("Team pool limit {}", money(cents_to_dollars(pooled_limit))),
    })
}
...
fn read_cursor_sqlite_token(path: &Path) -> Result<String, UsageError> {
    ...
    let token = serde_json::from_str::<String>(&raw).unwrap_or_else(|_| raw.trim().to_string());
    if token.is_empty() {
...
/// Read access token from the secondary JSON auth file (`auth.json`).
...
mod tests {
    ...
    #[test]
    fn standard_plan_reports_included_allowance_and_individual_cap() {
        let mapped = map_current_usage(Some("pro"), &period(PERIOD_PRO), None, &cycle(PERIOD_PRO))
            .expect("standard plan maps");

        assert_eq!(mapped.meters.len(), 2, "plan allowance + spend cap");
    ...
    fn enterprise_without_plan_usage_uses_individual_cap() {
        ...
        assert!(
            detail.contains("Team pool spend USD 1234.56"),
            "got {detail:?}"
        );
        assert!(detail.contains("of USD 5000.00"), "got {detail:?}");
    }
    ...
    #[test]
    fn sqlite_token_accepts_json_and_plain_values() {
    ...
    #[test]
    fn auth_json_accepts_camel_snake_and_plain_keys() {
    ...
    /// One loopback server that routes by path and records the paths it served
    /// in order. `responses` maps a path suffix to `(status, body)`.
    struct Loopback {
        port: u16,
        hits: Arc<Mutex<Vec<String>>>,
    }

    fn spawn_loopback(responses: Vec<(&'static str, u16, &'static str)>) -> Loopback {
        ...
        std::thread::spawn(move || {
            for request in server.incoming_requests() {
                ...
                hits_for_thread.lock().unwrap().push(url.clone());
                let override_response = responses
                    .iter()
                    .find(|(suffix, _, _)| url.ends_with(suffix))
    ...
    fn loopback_endpoints(port: u16) -> CursorEndpoints {
    ...
    fn hits(loopback: &Loopback) -> Vec<String> {
        loopback.hits.lock().unwrap().clone()
    }

    #[test]
    fn successful_current_flow_never_calls_the_legacy_endpoint() {
        let loopback = spawn_loopback(vec![
            (PLAN_INFO_PATH, 200, PLAN_ENTERPRISE),
            (PERIOD_USAGE_PATH, 200, PERIOD_PRO),
            (LEGACY_USAGE_PATH, 200, LEGACY_USAGE),
        ]);
        let usage = cursor_usage_with_token("test-token", &loopback_endpoints(loopback.port));
        ...
        let paths = hits(&loopback);
        assert!(paths.iter().any(|p| p.ends_with(PERIOD_USAGE_PATH)));
        assert!(
            !paths.iter().any(|p| p.ends_with(LEGACY_USAGE_PATH)),
            "legacy endpoint must not run when the current flow succeeds: {paths:?}"
        );
    }

    #[test]
    fn current_flow_failure_falls_back_to_the_legacy_endpoint() {
        let loopback = spawn_loopback(vec![
            (PLAN_INFO_PATH, 500, "{}"),
        ...
        let paths = hits(&loopback);
        let period_index = paths
            .iter()
            .position(|p| p.ends_with(PERIOD_USAGE_PATH))
            .expect("period usage attempted");
        let legacy_index = paths
            .iter()
            .position(|p| p.ends_with(LEGACY_USAGE_PATH))
            .expect("legacy fallback attempted");
        assert!(
            legacy_index > period_index,
            "legacy endpoint must run after the current flow: {paths:?}"
        );
    }

    #[test]
    fn unauthorized_current_flow_stays_logged_out_without_the_legacy_endpoint() {
        let loopback = spawn_loopback(vec![
            (PLAN_INFO_PATH, 200, PLAN_ENTERPRISE),
        ...
        assert!(
            !hits(&loopback)
                .iter()
                .any(|p| p.ends_with(LEGACY_USAGE_PATH)),
            "a rejected credential must not retry against the legacy endpoint"
        );
    }

    #[test]
    fn enterprise_uncapped_fetches_billing_cycle_aggregate_events() {
        let loopback = spawn_loopback(vec![
            (PLAN_INFO_PATH, 200, PLAN_ENTERPRISE),
        ...
        let paths = hits(&loopback);
        assert!(
            paths.iter().any(|p| p.ends_with(AGGREGATED_EVENTS_PATH)),
            "enterprise without planUsage must aggregate cycle events: {paths:?}"
        );
        assert!(!paths.iter().any(|p| p.ends_with(LEGACY_USAGE_PATH)));
    }

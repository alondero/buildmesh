...
/// workspace.
///
/// Thin wrapper over [`fetch_workspaces_against`] that pins the
/// production host. Tests use the parameterized form with a
...
/// HTTP-aware workspace fetch parameterized on the API host. Issue #967's
/// acceptance criterion #1 — round-trip test with mocked responses —
/// requires a server we can point at; the production path uses
/// [`OPENCODE_CONSOLE_HOST`] verbatim, the test path substitutes a
/// `tiny_http` listener bound to `127.0.0.1:0`.
///
/// Wraps `reqwest::blocking::Client` in a 15s timeout to bound the
/// network round-trip; matches the cadence `start_device_flow` uses.
pub(crate) fn fetch_workspaces_against(
    base_url: &str,
    access_token: &str,
) -> Result<Vec<OpenCodeWorkspace>, OAuthError> {
...
mod tests {
    ...
    // ── Round-trip HTTP tests (issue #967 acceptance criterion #1) ────────
    //
    // Exercise `fetch_workspaces_against` end-to-end against a `tiny_http`
    // server bound to a random loopback port. The production wrapper
    // `fetch_workspaces` pins `OPENCODE_CONSOLE_HOST`; the test wrapper
    // takes the host as a parameter so we never touch the live host from
    // CI. The `tiny_http` crate is already a regular `[dependencies]`
    // entry (it's the embedded test server for the Playwright e2e suite),
    // so this adds no new dep — it only puts an existing dep to work in
    // a unit-test context.

    /// Spawns a `tiny_http` server on a random loopback port. `handler` is
    /// invoked once per inbound request (caller matches on `req.url()`);
    /// the thread exits after `max_requests` so the test isn't held open
    /// by a blocking `accept()` after the assertions finish. Returns the
    /// port the client should dial.
        ...
        F: Fn(tiny_http::Request) + Send + 'static,
    {
        ...
        thread::spawn(move || {
            // Loop terminates when the listening socket is closed (server
            // dropped at closure exit) or after `max_requests` have been
            // served, whichever comes first. The counter guards the latter
            // so a request never sent by the client can't keep the thread
            // alive past the test function returning.
    ...
    #[test]
    fn fetch_workspaces_against_merges_user_first_then_orgs() {
    ...
    #[test]
    fn fetch_workspaces_against_dedups_user_workspace_when_listed_in_orgs() {
    ...
    #[test]
    fn fetch_workspaces_against_sends_bearer_authorization_header() {
        // Both `/api/user` and `/api/orgs` must carry the access token
        // via the `Authorization: Bearer <token>` header — otherwise the
        // Console host would 401 the workspaces RPC and the React
        // settings UI would never see the picker.
        use std::sync::mpsc;
        let (user_tx, user_rx) = mpsc::channel::<Option<String>>();
        let (orgs_tx, orgs_rx) = mpsc::channel::<Option<String>>();
        ...
        let base = format!("http://127.0.0.1:{port}");
        fetch_workspaces_against(&base, "test-token-xyz").unwrap();
        assert_eq!(
            user_rx.recv().unwrap().as_deref(),
            Some("Bearer test-token-xyz")
        );
        assert_eq!(
            orgs_rx.recv().unwrap().as_deref(),
            Some("Bearer test-token-xyz")
        );
    }

    #[test]
    fn fetch_workspaces_against_surfaces_user_http_error_as_shape() {
        // `/api/user` returning a non-success status is the
        // refresh-on-401 trigger surface (a 401 here means the cached
        // token is stale); we MUST surface it as `OAuthError::Shape` so
        // the higher layer can route on it instead of treating it as a
        // transport error (which would force a backoff, not a refresh).
        let port = spawn_loopback(2, |req| match req.url() {
            "/api/user" => {
                let _ = req.respond(
        ...
        let base = format!("http://127.0.0.1:{port}");
        let err = fetch_workspaces_against(&base, "tok").unwrap_err();
        assert!(
            matches!(err, OAuthError::Shape(_)),
            "expected Shape, got {err:?}"
        );
        assert!(
            format!("{err}").contains("/api/user HTTP 401"),
            "error string must identify the failing endpoint + status, got: {err}"
        );
    }

    #[test]
    fn fetch_workspaces_against_surfaces_orgs_http_error_as_shape() {
        // Mirror of the user-endpoint test for `/api/orgs`. The two calls
        // are independent — the React Settings UI relies on per-endpoint
        // error messages to render "couldn't load workspaces" affordances
        // that name the right path.
        let port = spawn_loopback(2, |req| match req.url() {
            "/api/user" => {
                let _ = req.respond(tiny_http::Response::from_string(
                    r#"{"id":"wrk_user","name":"User"}"#,
            ...
            "/api/orgs" => {
                let _ = req.respond(
                    tiny_http::Response::from_string(r#"{"error":"internal"}"#)

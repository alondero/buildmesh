...
**3. Layered, gracefully-degrading read model ([Node Digest](../../CONTEXT.md)).** An always-available spine from Buildmesh's own DB (lifecycle `status`, `needs_feedback` = `awaiting_input`, `waiting_since`) is enriched, **for the Claude Code provider family only**, with semantic content read from the JSONL transcript. A non-supporting provider, or a transcript that fails to parse, degrades to the spine with the enrichment **explicitly flagged `unavailable`** — never silently omitted, so the Coordinator can tell "node is quiet" from "rich layer is down". The rendered terminal/TUI is deliberately **not** a read source.

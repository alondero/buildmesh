...
5. **Two pages, with Usage Meters promoted to a glanceable surface (issue #601).** A provider-centric **Providers** page owns credentials (canonical key editor); a harness-centric **config** page reorders harnesses and attaches proxied providers. **Usage Meters live on the Probe Panel's dedicated `'usage'` tab** (alongside Files / Issues / PRs / Archive), reachable in one click from the always-visible activity bar. The Settings modal's "Providers" section becomes purely config (no bars), and the Usage tab is read-only — no edit affordances. Usage is **detection-gated**: a harness-native subscription meter shows when the harness is installed (no key needed); a keyed provider's meter shows when its key is set; an uninstalled harness's meter is never shown. A provider may have **more than one Usage Meter** (e.g. Anthropic subscription *and* API wallet); proxying one credential through many harnesses is still one meter. Cross-surface invalidation: the Rust backend emits `provider-list-changed` on upsert/remove, and `useProviderListInvalidation` re-fetches the Usage tab so a toggled or removed provider's meter updates without a manual Refresh.
...
- **The OpenAI/Codex surface is wired and live-verified on `codex-cli 0.144.0`** (issue #599, 2026-07-18). Live probing established the contract:
  - `OPENAI_API_KEY` **is** honoured by codex as an env var (its `auth.credentials` reports it under `auth env vars present`). Kept as an env var injection on the spawned child.
  - `OPENAI_BASE_URL` / `OPENAI_MODEL` are **silently ignored** by codex as env vars — even with `OPENAI_BASE_URL=https://api.minimax.io/v1` exported, `codex doctor --json` reports `endpoint: wss://api.openai.com/v1/<redacted>`. `-c key=value` overrides also reject custom `model_provider` values (only the built-in `openai` parses), so the effective routing happens via codex's profile mechanism: `agent::spawn::build_spawn_command` derives a per-pairing profile name from the same `OPENAI_*` pairs via `codex::proxy_pair`, idempotently writes `$CODEX_HOME/<name>.config.toml` (a `[model_providers.<name>]` block codex actually consumes) via `codex::ensure_proxy_profile`, and passes `-p <name>` to the codex CLI. Native Codex (`backend_env` carries no `OPENAI_BASE_URL`) is regression-pinned by `proxy_pair_none_for_native_codex`. The Anthropic surface continues to be exercised end-to-end.
- **Amendment (2026-07):** an **Anthropic-surface** pairing's `base_url`/`model_tiers` are no longer honoured from a stored pairing — they are re-derived live from the account on every resolution (account edits win; the published first-class endpoint fills anything the account leaves empty). Rationale: the attach flow stored a *snapshot* of the derived defaults, and since no UI edits a pairing's tiers, the snapshot could only go stale — Providers-page edits (e.g. Kimi model tiers) silently never reached newly spawned sessions. A stored pairing still records *that* the attachment exists, and non-Anthropic (OpenAI/Codex) pairings keep their stored payload.

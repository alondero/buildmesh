...
4. **Startup Auto-Detection (Scan on Init)**: On startup, scan the system `PATH` and standard directories (like `~/.claude/` or `~/.codex/`) to detect installed tools and existing credential configurations. Only auto-populate and enable harnesses/providers that are actually detected on the host machine. *(Implementation, issue #536: the scan is a dep-free in-process `stat` sweep — a few hundred cached lookups, a couple of ms — run synchronously in `setup()` via `agent::detection`. The merge into `preferences.json` is additive and idempotent, so it runs on **every** launch rather than first-run-only: newly-installed tools are picked up and a user's renamed/customised profiles are never overwritten.)*

...
- **Refinement history:** The original decision (issue #213) hardcoded `origin`; issue #276 made the remote `base_ref`-driven with a three-tier fallback. The fast-forward + `--no-rebase` contract is unchanged. **Superseded in part by ADR 0022** (2026-07-17): the "skip if dirty" criterion now gates only the fast-forward pull — the fetch always runs (it never touches the working tree, and the remote-tracking refs it updates are what worktree nodes are cut from). `SkippedDirty` became `FetchedButDirty { new_commits }`. **Further refined by ADR 0033** (2026-09): even the pull gate is no longer "is the tree dirty?" — it is the precise set of paths the pull would overwrite. Local edits and untracked files in paths the incoming commits don't rewrite no longer block the fast-forward, and `FetchedButDirty` carries the blocking `blocking_paths`.

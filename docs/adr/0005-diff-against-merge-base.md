...
The Agent Changes / diff views compute an Agent Node's changes against the **merge-base of its worktree `HEAD` and the mesh's `base_ref`** (default `origin/main`), not against `HEAD`. The "old" side of every diff is the merge-base tree; the "new" side is the working directory, so a file is shown as changed if the agent touched it **at any point since branching** — whether the change is committed or still uncommitted.

## Context

Before this decision, `diff_file_against_head` and the desktop Project Files Changed Files list diffed the working tree against `HEAD`. That answers *"what is uncommitted right now?"* — but it is the wrong question for reviewing an agent. The mobile Agent Changes view also had a mixed baseline: its file list was HEAD-relative while its tapped diff was base-relative.
...
2. **The change set and file diffs are separate reads.** `node_changed_files(node_id)` enumerates every changed file with its status and `+/−` counts without constructing hunks. The desktop Agent Changes probe renders that lightweight list; clicking a row then calls `diff_node_file_against_base` for the single file in the centre workspace. `diff_node_against_base` remains available for callers that need the complete `DiffResult`, while the interactive desktop path avoids paying for every hunk during initial probe render.
...
- **The initial panel stays light.** Opening the panel fetches file paths and line counts only; clicking a file makes one per-file diff call and displays the result in the centre workspace.

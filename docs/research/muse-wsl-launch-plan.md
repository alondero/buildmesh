...
The local implementation now performs the preflight in provisioning, and the direct production-wrapper test exercises that operation before a real WSL `muse exec` process. The earlier disposable PTY probe also established fresh and resume feasibility at the Windows-to-WSL process boundary. This does **not** establish Tauri command/DB orchestration, UI launch, linked-worktree repair, arbitrary project layouts, a real host without symlink privilege (covered only via the deterministic creation-fault seam in the checked-in suite), or normal shell write operations. Authenticated rule reading and project skill discovery were validated separately from the echo PTY lifecycle test. The focused Rust tests, Clippy, agent check, and diff checks have been run; a full WebView2 UI smoke test remains outside this validation.

The checked-in Windows regression suite is `cargo test --locked --manifest-path src-tauri/Cargo.toml git::ai_context_runtime::tests::windows_tests --no-fail-fast`: fourteen tests pass (including a deterministic no-privilege case that denies link creation and asserts an intact checkout plus a capability error) and three live tests are ignored by default. The live tests were executed separately with `--ignored --nocapture`: the WSL variant repairs a disposable Windows checkout, invokes the actual `spawn_environment::wrap` command builder used by Buildmesh, runs `muse exec --provider echo`, and confirms both links plus clean Git status on both the Windows and WSL views afterward; the native variant mirrors it through the production `muse.exe` recipe (`Platform::Windows`, `EnvType::Windows`); a third live test asserts a natively-repaired checkout is clean from both Windows and WSL Git. This is real harness startup evidence, not authenticated Meta inference; the authenticated marker result is recorded separately in the table above. The repository gate `scripts/check.ps1 rust -SerialRust` also passed; Clippy exits successfully and the repository emits warnings outside the touched module.
...
If a post-install check fails, fail closed: remove and restore only an exact link owned by this operation, using the directory-aware removal required by `.agents/skills`; restore its validated backup with no-replace semantics. If the path is an unexpected object, preserve it and leave the backup and transaction manifest for the next recovery pass. This preserves an external replacement even across a check/delete race, while allowing a failed rollback to converge when the owned link is dangling.
...
The current Windows regression suite now explicitly covers cached-index refresh, missing skills targets, directory-symlink candidate cleanup after a failed pre-install, recovery of a dangling owned directory link after failed rollback, blocked rollback recovery with backup preservation, concurrent launch serialization while one launch holds the lock, and denied symlink privilege via a deterministic creation-fault seam (intact checkout plus a privilege/Developer Mode error, then successful repair on retry). External replacement during install remains release work where the host provides no deterministic seam.

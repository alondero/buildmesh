...
endpoints and prompts are excluded from the configuration history summary. A
continued review names the run it follows in its own history, so the link
survives retention compacting the run's context.

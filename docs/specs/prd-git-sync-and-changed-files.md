...
2. **Auto-Sync on Node Spawn:** Automatically fetch and attempt to fast-forward the Mesh branch before spawning a new Agent Node when the fast-forward would not overwrite local changes, falling back gracefully to local HEAD if the sync fails.
...
- **Backend Spawn Logic (`src-tauri/src/agent/spawn.rs`):**
  ...
  - Perform `git pull --ff-only` on the parent repository during spawn when it would not overwrite local changes (ADR 0033 — local edits and untracked files in paths the incoming commits don't touch no longer block it). Fall back with a logged warning if pulling fails.

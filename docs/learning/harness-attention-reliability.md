...
| MiniMax Code (`mcode`) | No attention hooks; readable `messages.jsonl` transcripts | Installed 0.2.7 contains two hook systems with different event catalogs. Native hook capability exists, but a validated TUI delivery/config route is still required. Transcripts are wired (`TranscriptFormat::Mcode`): digest, picker, and circuit reports work; Autopilot still gates on the missing hook. |

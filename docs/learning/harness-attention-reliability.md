...
| MiniMax Code (`mcode`) | No attention hooks shipped to users; readable `messages.jsonl` transcripts | The 0.4.x Agent-Plugin wiring (`io.buildmesh.attention` plugin installing `Stop` / `PermissionRequest` curl entries, additive merge, atomic write, malformed-refusal, stdin entrypoint test) is implemented and unit-tested but **not yet invoked from spawn** — `requires_attention_hook` stays `false` per issue #1796's acceptance line until TUI delivery is validated. Follow-up flips the descriptor; until then users do not see the plugin on disk. Transcripts are wired (`TranscriptFormat::Mcode`): digest, picker, and circuit reports work; Autopilot still gates on the descriptor. |
...
| MiniMax Code, Freebuff, DeepSeek Harness, Terminal | Explicit capability gaps remain | mcode's `Stop` / `PermissionRequest` plugin wiring ships but the descriptor stays `false` pending TUI delivery validation (issue #1796 acceptance). Freebuff, DeepSeek Harness, and Terminal still need a validated plugin, MSP/log, profile, or protocol integration before Buildmesh can promise parity. |

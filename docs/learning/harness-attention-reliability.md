...
Muse's installed Ubuntu CLI returned **1.1.1 (1.1.1-R2514.1)**. Its help exposes `serve` (MSP), `schema`, `trace`, `export`, `--no-session-log`, and a deterministic `echo` provider. Those are feasible seams for unpaid lifecycle experiments; none establishes a hook config. Buildmesh launches with `--disable-approval` (plus `--disable-sandbox`, issue #1788), which does not establish whether questions or non-tool prompts can occur. Existing [interop research](windows-wsl-harness-interop.md) records the log/index paths. [Meta documentation entry point](https://dev.meta.ai/docs/muse-code)

...
| MiniMax Code (`mcode`) | No attention hooks; readable `messages.jsonl` transcripts | Installed 0.2.7 contains two hook systems with different event catalogs; the 0.4.x Agent-Plugin surface (`hooks/hooks.json` + `Stop` / `PermissionRequest`) is the version the Buildmesh plugin targets. Native hook capability exists, but a validated TUI delivery/config route is still required. Transcripts are wired (`TranscriptFormat::Mcode`): digest, picker, and circuit reports work; Autopilot still gates on the descriptor until the TUI delivery follow-up lands. |
...
| MiniMax Code, Freebuff, DeepSeek Harness, Terminal | Explicit capability gaps remain | mcode's `Stop` / `PermissionRequest` plugin is now provisioned; the descriptor stays `false` until TUI delivery is validated end-to-end. Freebuff, DeepSeek Harness, and Terminal still need a validated plugin, MSP/log, profile, or protocol integration before Buildmesh can promise parity. |

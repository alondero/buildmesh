...
| Cline | Buildmesh-provisioned file hooks (`~/.cline/hooks/{TaskComplete,SessionShutdown}`); no transcript reader | Cline 3.0.62 resolves file hooks by exact event name additively from four fixed directories. Buildmesh provisions the user-global root with the two events it can normalise — `agent_end` (turn completion) and `session_shutdown` (session exit). No permission/question/background primitive exists under the default auto-approve launch. See [cline-harness-capabilities.md](cline-harness-capabilities.md). |
...
| Cline | `TaskComplete`/`SessionShutdown` file hooks → turn completion (`Ready`) and session exit (`Idle`) | High on Cline 3.0.62 with the provisioned global hooks (`agent_end` fires per completed run, `session_shutdown` on teardown). Permission, question, and background signals are impossible by construction under the default auto-approve launch, and the transcript reader is not wired (issue #1776). |

...
| Grok | Global command hooks (curl to loopback) | Direct completion/notification path exists. Error/cancel outcomes and late callbacks need explicit handling. Question tool is `ask_user_question`. |
...
| MiniMax Code (`mcode`) | Buildmesh-provisioned attention hook (`io.buildmesh.attention`); readable `messages.jsonl` transcripts | The 0.4.x Agent-Plugin wiring (`io.buildmesh.attention` plugin installing `Stop` / `PermissionRequest` curl entries, additive merge, atomic write, malformed-refusal, stdin entrypoint test) is implemented, unit-tested, and **invoked from spawn**. Issue #1797 validated `Stop` delivery against a live 0.4.12 TUI (2026-09-20) and flipped `requires_attention_hook` to `true`, so the plugin is installed for users. The validation also fixed the plugin layout: mcode 0.4.0+ loads only `.claude-plugin/plugin.json` with `hooks` inlined, and silently skips a directory with no manifest. `PermissionRequest` is provisioned but not advertised — the launch auto-approves (`"permission_mode": "auto"`). See [mcode-harness-capabilities.md](mcode-harness-capabilities.md). Transcripts are wired (`TranscriptFormat::Mcode`): digest, picker, and circuit reports work; Autopilot is now allowed. |
| Meta Muse | Passive watcher over the durable session log; readable event storage used for session/usage support | Installed 1.1.1 had structured logs and an MSP server but no interactive hook registration. Muse **1.3.0** adds a claude-compatible plugin hook surface, but it is gated behind an explicit `muse plugins approve` into a global cache (and a node-local install needs workspace trust), so it is not provisioned; the passive watcher remains the wired signal. |
| Cline | Buildmesh-provisioned `TaskComplete` file hook (`~/.cline/hooks/`); no transcript reader | Cline 3.0.62 resolves file hooks by exact event name additively from four fixed directories. Buildmesh provisions the user-global root with the only event its file-hook layer delivers on a clean run — `agent_end` (turn completion). There is no clean-exit dispatch (`session_shutdown` is abort-only), and no permission/question/background primitive exists under the default auto-approve launch. See [cline-harness-capabilities.md](cline-harness-capabilities.md). |
...
| Grok | Stop, idle/task notifications, prompt submission, failures/cancellation, and `ask_user_question` pre/post/failure | High when the command hook is provisioned and the runtime token is presented; callbacks from another process are rejected. |
...
| Meta Muse | Passive watcher over the durable `session.jsonl` run boundaries (issue #1709); contract re-verified unchanged on Muse 1.3.0 | Medium completion confidence; `--disable-approval` makes a permission signal impossible by construction, and no question observer is claimed. A native plugin `Stop` hook exists on 1.3.0 but is not provisioned — see the [`### Muse`](#muse) section. |
| Cline | `TaskComplete` file hook → turn completion (`Ready`) | High on Cline 3.0.62 with the provisioned global hook (`agent_end` fires per completed run). The file-hook layer has no clean-exit dispatch (`session_shutdown` is abort-only, so it never signals teardown), no permission/question/background signal under the default auto-approve launch, and no provisioned failure signal — a clean exit is observed through PTY EOF as before, and a failed run is bounded by the existing watchdog. The transcript reader is not wired (issue #1776). |
| MiniMax Code, Freebuff, DeepSeek Harness, Terminal | Explicit capability gaps remain | mcode's `Stop` hook is live (validated on installed 0.4.12, 2026-09-20): the plugin loads from `.claude-plugin/plugin.json` and the descriptor is `true`. Freebuff, DeepSeek Harness, and Terminal still need a validated plugin, MSP/log, profile, or protocol integration before Buildmesh can promise parity. |
...
The same guide warns that Stop is a gate: another hook can cause a continuation without another user prompt. The installed `03-keyboard-shortcuts.md` identifies the question tool as `ask_user_question`. Buildmesh uses platform-specific command hooks that forward stdin JSON with curl because Grok rejects `http://` URLs in native HTTP hooks; the loopback listener intentionally serves plain HTTP. Callback delivery still depends on loopback reachability across Windows/WSL, and the URL carries the runtime hook token. Public entry point: [Grok CLI reference](https://docs.x.ai/build/cli/reference). These detailed claims derive from the installed first-party guides and adapter contract, not a live model test.
...
Muse **1.1.1** had no interactive hook registration: `muse --help` exposed no hook/event flag and there was no workspace or global hook config file. `muse schema generate-json-schema` proves the agent loop *does* carry the lifecycle vocabulary — `turn/started`, `turn/completed` (`terminal: completed|failed|cancelled`), `approval/{requested,resolved,updated}`, `userInput/{requested,settled}` — but those are served only on the separate `muse serve` stdio MSP plane, not by the interactive TUI Buildmesh PTY-spawns.

Muse **1.3.0** (re-probed 2026-09-21; the version Buildmesh supports today) adds a **claude-compatible plugin hook surface**. `muse plugins validate` accepts a `.claude-plugin/plugin.json` bundle (`manifest_family: "claude-compatible"`) whose `Stop`, `Notification`, `PermissionRequest`, `UserPromptSubmit`, `SessionStart`, `SubagentStop`, `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PreCompact` and `SessionEnd` handlers classify as supported; a live `Stop` (offline `muse exec --provider echo`) delivers the same `hook_event_name`/`session_id`/`turn_id` payload `http::routes::attention` already maps to a clean turn completion.

It is nevertheless **not provisioned**, for three compounding reasons: `muse plugins install` copies the bundle into the user's **global** plugin cache (`~/.local/share/muse/plugins/`) rather than the node's worktree; a third-party bundle's hooks stay at `review_needed` with `effective_capabilities: []` until an explicit `muse plugins approve` — a consent step Buildmesh would have to bypass unattended; and the node-local `--scope project` install is refused until the workspace is trusted (issue #1706). Muse also does not pass `BUILDMESH_*` to hooks, so the callback URL would have to be baked per node, re-approving and growing the global cache on every spawn. The passive watcher therefore stays the wired signal, and the durable-log contract it depends on is **unchanged** on 1.3.0 (re-verified: 58 `started` / 57 `terminal` across 27 retained top-level logs). Wiring the hook is a follow-up once #1706 lands.
...
Buildmesh launches Muse with `--disable-approval`. Across every retained
`approval_disabled` session log there were **zero** `approval/requested` records (all observed approval requests came from `security_mode == "normal"` interactive sessions), so `PermissionRequested` is impossible by construction and is not classified. Muse additionally ships its own OS sandbox, ON by default alongside approval ("Safety (approval and the sandbox are ON by default)"); Buildmesh bakes `--disable-sandbox` so the agent shell keeps the same OS reach every other harness has — the sandbox denied access to the OS credential keyring and broke gh/git auth (issue #1788). Full findings and the schema/payload inventory: [muse-attention-signals.md](../research/muse-attention-signals.md).

### Remaining harnesses

Installed MiniMax Code package **0.2.7** has a first-party changelog saying lifecycle plugin hooks arrived in 0.2.4 and tool-name/input compatibility fixes in 0.2.5. The **0.4.12** bundle confirms the hook wire shape (stdin JSON with `hook_event_name`, session/turn ids, `transcript_path`, `cwd`; stdout JSON decisions) and the session store (`<dataDir>/v2/sessions/…/messages.jsonl`, manifest-indexed) that `TranscriptFormat::Mcode` now parses — see [mcode-harness-capabilities.md](mcode-harness-capabilities.md). Hook provisioning (the Autopilot unlock) is validated and live: the issue #1797 run against installed 0.4.12 (2026-09-20) delivered `Stop` from both `mcode exec` and the interactive TUI, and the plugin loads from `.claude-plugin/plugin.json` with `hooks` inlined — mcode silently skips a directory with no manifest, which is exactly what the earlier `hooks/hooks.json`-only layout tripped over. Its packaged `chunks/chunk-BPRXRI4W.js` lists `Stop` and `PermissionRequest`; `chunk-CSEMCTUO.js` has a separate daemon hook catalog including `MessageComplete`, and warns unsupported events will never fire. Do not register Stop into the latter and claim success. That second bundle also implements compatible plugin manifests (`.claude-plugin/plugin.json`, `.codex-plugin/plugin.json`); the 0.4.0+ runtime reads `hooks` **inline** on `.claude-plugin/plugin.json` and ignores the separate `hooks/hooks.json` document — both established by the live issue #1797 run. The first-party portable example is explicitly a structural preview, not a proven integration. [MiniMax plugin example](https://raw.githubusercontent.com/MiniMax-AI/MiniMax-Code-Plugins/main/examples/hello-mcode-hooks/README.md), [preview hook layout](https://raw.githubusercontent.com/MiniMax-AI/MiniMax-Code-Plugins/main/examples/hello-mcode-hooks/io.minimax.mcode/hooks/hooks.json)

Muse's installed CLI is **1.3.0 (1.3.0-R3401.1)** — the native Windows binary at `%LOCALAPPDATA%\Programs\muse\muse.exe`, with a 1.1.1 Ubuntu build as the WSL fallback. Its help exposes `serve` (MSP), `schema`, `trace`, `export`, `plugins`, `--no-session-log`, and a deterministic `echo` provider: the seams used for the unpaid lifecycle experiments above. `muse plugins install|approve` is now a real but consent-gated and global hook path — see the `### Muse` section. Buildmesh launches with `--disable-approval` (plus `--disable-sandbox`, issue #1788), which does not establish whether questions or non-tool prompts can occur. Existing [interop research](windows-wsl-harness-interop.md) records the log/index paths. [Meta documentation entry point](https://dev.meta.ai/docs/muse-code)

...
- `~/.local/share/muse/session-index.db` has a `sessions` table with `session_id`, `session_log_path`, `workspace_root`, `workspace_key`, `created_at_us`, and `updated_at_us`. The existing observed row had all four workspace/time fields null. The JSONL metadata supplies the missing workspace and timestamp; capture must not require the index fields to be populated. The index is also written asynchronously: a *live* session has no row at all until a later Muse process flushes it, so a lookup that needs the session identity or log path must read the `sessions/YYYY/MM/DD/<uuid>/session.jsonl` tree as well (circuit run 183 — see `docs/development/circuit-run-183-muse-index.md`).
...
- Installed Muse Code 1.3.0 on Ubuntu and Windows: CLI help/parser and metadata schema checked on both; the Windows binary accepts `--disable-approval` and `--disable-sandbox` (baked together by the spawn recipe, issue #1788) and the spawn recipe branches to `muse.exe` under `WindowsShell::Direct`. Authenticated Meta inference and interactive resume were not exercised.

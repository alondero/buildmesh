...
- **Pitfalls** — each caught by an iteration of real bugs:
  ...
  4. **The Rust blob format is implicit.** Buildmesh's parser pins the four-field shape; a future extension that adds a fifth field must update both the writer (`services::opencode_oauth::persist_token_response`) AND the parser (`services::opencode_oauth::parse_opencode_console_full_credential`) atomically, or the live probe will silently ignore the new field (serde default = drop-unknown on the wire).

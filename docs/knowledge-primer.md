...
### Keyboard Shortcut Disambiguation (issue #1409)
The Omnibar claims `CommandOrControl+K` / `CommandOrControl+P` on macOS, and `CommandOrControl+Shift+K` / `CommandOrControl+Shift+P` on Windows/Linux, as Tauri global shortcuts — captured at the OS layer before xterm sees the keydown. macOS `⌘+K` was previously terminal clear — that moved to `⌘+Shift+K` (matching Windows/Linux `Ctrl+Shift+K`). The platform-canonical modifier split (meta on Mac, ctrl elsewhere) lives in `terminalKeyAction.ts` (`resolveKeyAction` / `resolveZoomKeyAction`); the display labels live in `shortcutCatalog.ts`; the App.tsx `shortcuts` array is the binding source of truth. The open/close state lives in `uiStore` (`omnibarOpen` / `toggleOmnibar`, map #1371 Decision #2). When adding a global shortcut that could collide with an xterm chord, remap the terminal side or augment the modifier rather than stealing the user's keystroke.
...
- **Issue-driven Autopilot + PR review blueprint:** `CircuitBlueprintKind::IssueDrivenAutopilotReview` is server-owned and requires a GitHub issue-label trigger plus a PR-producing mesh policy. It reuses the issue Autopilot finish prompt and collaborator gate, then requires the implementation agent's pushed branch/PR before spawning a separate reviewer. The exact reviewer prompt is `CircuitGraph::PR_REVIEW_PROMPT`; the reviewer receives both `{{pr.number}}` and `{{pr.url}}`, its terminal report is persisted as `node.reviewer.output`, and the circuit injects that feedback into the implementation agent before closing the reviewer node. A bounded `RetryLimit` loops back through the implementation and review path, with `retry.*` and `verification.*` context populated as those gates run; exhaustion emits the normal circuit notification. The reviewer has its own worktree and the circuit minimum concurrency is 2, while the mesh/global Autopilot capacity gates still apply. Mustache autocomplete and the inspector drawer only expose context paths reachable from the selected node's upstream graph.
- **Circuit-agent ownership:** circuit-spawned agents are registered in the shared evaluator but do not create `autopilot_runs` rows. The circuit worker owns their turn classification, global/per-mesh capacity accounting, GitHub-action replay after a restart, and best-effort cleanup when a run fails.

...
**Autopilot Circuit**:
A user-authored trigger-action graph (a small DAG) on one Mesh — the composable generalisation of the two fixed Autopilot modes. The **blueprint** persists as `graph_json` on the `autopilot_circuits` row; a **Circuit Run** is one execution instance of it (`autopilot_circuit_runs`, deduped per `(circuit, trigger_identity)`); a **Circuit Step** is one circuit node's execution within a run (`autopilot_circuit_run_steps`; `pending_slot` = parked on a concurrency limit). Decisions come from a pure stepper (`autopilot::circuit::stepper`); the worker thread in `services::circuit_worker` observes live state and executes its effects. "Node" is overloaded: always qualify — *circuit node* (graph vertex) vs *agent node* (mesh session).

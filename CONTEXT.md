...
**Autopilot Circuit**:
A user-authored trigger-action graph on one Mesh — the composable generalisation of the two fixed Autopilot modes. Cycles are allowed only when every directed cycle is bounded by a `RetryLimit` or a `CollaboratorCheck` that requires approval. Newly created circuits start **disabled** (draft-first) so background pollers cannot fire while the graph is still being authored; **Trigger Now** still dry-runs a disabled circuit. The **blueprint** persists as `graph_json` on the `autopilot_circuits` row; a **Circuit Run** is one execution instance of it (`autopilot_circuit_runs`, deduped per `(circuit, trigger_identity)`); a **Circuit Step** is one circuit node's execution within a run (`autopilot_circuit_run_steps`; `pending_slot` = parked on a concurrency limit). A step owns the Agent Node and every child agent or background task it launches; ownership is explicit, and work does not silently detach from the step. **Verified completion** requires the foreground work and every owned child or background task to have terminal evidence; a foreground yield or process exit alone is not enough. A classifier verdict interprets whether the task is complete or classifies the specific fresh review report it evaluated; it does not prove that work has terminated or satisfy separate permission or human-approval gates. If required evidence is unknown, the step remains an actionable **Unverified** checkpoint rather than auto-advancing. A harness's advertised capabilities describe possible observation sources; **Observation Confidence** records what Buildmesh actually established at runtime, distinguishing verified, degraded, unverified, and unavailable evidence. **Stable Work Identity** is a persisted key that matches a work item across callbacks and restarts, fenced by its run, step, and attempt; a PID, PTY text, timestamp, or in-memory correlation is not sufficient. An owned human-input request puts the step in **Needs input** without failing it; a clean parent yield cannot hide a pending child question, and permission, ordinary question, and review approval remain distinct. **Needs input** is a non-failing, potentially indefinite wait that ends only through the matching response, explicit pause or cancellation, confirmed work loss, or an operator-recorded outcome. **Observation Strategy** is harness-owned and ordered: a validated explicit pushed lifecycle hook is authoritative for the lifecycle fact it declares, then a harness-specific validated pull/reconciliation method may establish facts the hook does not cover; absent a validated strategy, evidence is unavailable rather than guessed from generic silence. A foreground completion hook does not override outstanding owned child or background work. **Missing lifecycle observation** enters a bounded, adapter-specific reconciliation path; if it cannot establish a current fact, the step becomes an actionable Unverified checkpoint rather than silently failing or waiting forever. An unsupported hook is an unavailable capability, while a stale or mismatched callback is a rejected observation. **Restart reconciliation** re-establishes ownership and evidence after an app restart; only identity-proven work may be reattached, while ambiguous work remains Unverified and never triggers an automatic replay. Decisions come from a pure stepper (`autopilot::circuit::stepper`); the worker thread in `services::circuit_worker` observes live state and executes its effects. The ledger vocabulary above is the **stored** vocabulary; user-facing surfaces render a humanised label instead (`pending_slot` → "Queued", `blocked` → "Needs approval", `completed` → "Done") via `Circuits/runDiagnostics.ts`, keeping the raw token on a `data-run-state` / `data-step-status` attribute for tests and CSS (issue #1468). "Node" is overloaded: always qualify — *circuit node* (graph vertex) vs *agent node* (mesh session).

...
Pending Circuit Runs have a persisted, per-Mesh queue position and are admitted nearest-first; users may move or cancel them. The Mesh agent limit gates a run's first Agent Node only, so an admitted issue-review run can spawn its reviewer even while its implementer is retained. Cancellation terminalises the run before attached Agent Nodes are retired; deleting a Circuit disables it and performs that same cleanup before deleting its ledger.

...
Pending Circuit Runs have a persisted, per-Mesh queue position and are admitted nearest-first; users may move or cancel them. Before admission, the worker reserves the blueprint's declared SpawnAgentNode footprint in a durable per-run lease; every spawn remains bounded by the Mesh and global host process caps, so a run whose footprint cannot fit stays queued. Cancellation terminalises the run before attached Agent Nodes are retired; deleting a Circuit disables it and removes its ledger only after every retirement succeeds, retaining the disabled ledger for retry when cleanup fails.

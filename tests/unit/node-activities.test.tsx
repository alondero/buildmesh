...
vi.mock('../../src/components/Terminal/BuildRunTerminal', () => ({
  BuildRunTerminal: ({ sessionId, mode, onClose }: { sessionId: number; mode: string; onClose: () => void }) =>
    <div><span>{`${mode} output ${sessionId}`}</span><button onClick={onClose}>Close utility</button></div>,
}));
vi.mock('../../src/components/AgentNodeView/GridNodeHeader', () => ({
  GridNodeHeader: ({ nodeId, onBuildRun }: { nodeId: number; onBuildRun: (id: number, mode: string) => void }) =>
    <div><span>Controls {nodeId}</span><button onClick={() => onBuildRun(nodeId, 'terminal')}>Open terminal</button></div>,
}));

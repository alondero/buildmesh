...
export default async function ({ page }) {
  ...
  await expect(page.getByTestId('circuits-view-activity')).toHaveAttribute('aria-pressed', 'true');
  await expect(page.getByTestId('circuit-name-input')).toHaveCount(0);
  await expect(page.getByTestId('queue-run-1002')).toHaveCount(0);
  await expect(page.getByTestId('run-toggle-1001')).toHaveAttribute('aria-expanded', 'false');
  await page.getByTestId('run-toggle-1001').focus();
  await page.keyboard.press('Enter');
  await expect(page.getByTestId('run-error-1001')).toBeVisible();
  await expect(page.getByText('Debug details')).toBeVisible();
  await page.getByText('Debug details').click();
